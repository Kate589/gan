...
classes = np.load("datasets/wikiart-raw/classes.npy")

def generate_dataset_main(n=10000, save_path=None, seed=None, model_res=1024, image_size=256, minibatch_size=16, truncation=0.7, labels=None):
    """
...
def generate_dataset(n=10000, save_path=None, seed=None, model_res=1024, image_size=256, minibatch_size=16, truncation=0.7, classes=None):
    """
    ...
    labels = random.choice(classes)
    W, X = generate_dataset_main(inc, save_path, seed, model_res, image_size, minibatch_size, truncation, labels)
    for i in range(batch_size-2):
        labels = random.choice(classes)
        aW, aX = generate_dataset_main(inc, save_path, seed, model_res, image_size, minibatch_size, truncation, labels)
        W = np.append(W, aW, axis=0)
        aW = None
        X = np.append(X, aX, axis=0)
        aX = None
    labels = random.choice(classes)
    aW, aX = generate_dataset_main(left, save_path, seed, model_res, image_size, minibatch_size, truncation, labels)
    W = np.append(W, aW, axis=0)
    aW = None
    X = np.append(X, aX, axis=0)
    aX = None
...
def finetune_resnet(model, save_path, model_res=1024, image_size=256, batch_size=10000, test_size=1000, n_epochs=10, max_patience=5, seed=0, minibatch_size=32, truncation=0.7):
    ...
    np.random.seed(seed)
    W_test, X_test = generate_dataset(n=test_size, model_res=model_res, image_size=image_size, seed=seed, minibatch_size=minibatch_size, truncation=truncation, classes=classes)
    ...
    while (patience <= max_patience):
        ...
        W_train, X_train = generate_dataset(batch_size, model_res=model_res, image_size=image_size, seed=seed, minibatch_size=minibatch_size, truncation=truncation, classes=classes)
...
parser.add_argument('--truncation', default=1, help='Generate images using truncation trick', type=float)
